module Plugins::CamaLanguageEditor::MainHelper
  ...
  def cama_language_plugin_render_item_translations(groups, base_key = nil, res = [], p_keys = [])
    res_html = groups.map{|g| "" }
    all_attrs = PluginRoutes.static_system_info['available_languages'].map{|_l| cama_language_plugin_get_hash_value(I18n.backend.send(:translations)[_l.to_sym], base_key.split(':') + p_keys) }
    all_attrs.map{|g| g.keys || [] }.flatten.uniq.sort_by{|k| groups.map{|_g| _g[k].is_a?(Hash) ? 1 : 0 }.reduce(&:+) }.each do |key|
      ...
      groups.each_with_index do |g, index|
        ...
        _g = p_keys.present? ? cama_language_plugin_get_hash_value(res[index], p_keys.dup) : res[index]
  ...
  def cama_language_plugin_get_hash_value2(g, keys)
    k = keys.shift
    (keys.any? ? cama_language_plugin_get_hash_value(g[k] || {}, keys) : g[keys]) || {}
  end
end
